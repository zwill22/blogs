...
to retrieve real-time bus locations.
...
Finally, 5 accessor functions are included to retrieve the members of `Authentication` from memory.
...
properly initialised. This responsibility is passed to the user of the library.
...
Finally, the uninitialiser is called once the client is no longer needed.
...
logic to the uninitialiser, converting the raw pointer to a pointer of the class type and then calling the
...
Let's run through an example C++ `main` function, the full example file is [available here](https://github.com/zwill22/OpenID/blob/main/examples/CInterface.cpp).
...
void * idProvider = allocaÛ(idProviderSize());
...
However, I will repeat a couple of points where relevant.
...
Similar the `OpenID::IDProvider` class, the initialiser uses the `IDSettings` to create an instance of the class.
...
struct. Finally, the class must include a uninitialiser to clean-up the pointers and call their destructors.
